...
k1.metric("Active Alerts",      total_alerts)
k2.metric("HIGH or CRITICAL",   high_crit)
k3.metric("Suppliers Affected", total_sup)
k4.metric("Total Cost Exposure", f"${total_cost:.0f}M")
...
with log_col:
    st.markdown(
        '<div style="color:#e8eaed;font-size:16px;font-weight:700;margin-bottom:14px;">⌨️ Event Log</div>',
        unsafe_allow_html=True,
    )
    ...
    with log_container:
        for alert in sorted(alerts, key=lambda x: x["detected"], reverse=True):
            ...
            st.markdown(
                ...
                f'[Risk:{score}]</span>'
                f'<span style="color:#a0aab4;font-size:11px;line-height:1.4">{emoji} {alert["title"]}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
